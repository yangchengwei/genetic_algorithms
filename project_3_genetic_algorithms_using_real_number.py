...
XX = new_x1
YY = new_x2
if (XX)/(math.sqrt(3))-YY<0 or -XX-math.sqrt(3)*YY+6<0 or YY<0:
    print('out of range !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    print('NEWX,NEWY  ',XX,YY)
    print('X1,X2      ',self.x1, self.x2)
    print('NEWRX,NEWRY',new_rate1, new_rate2)
    print('R1,R2      ',self.rate1, self.rate2)
'''
import math
import time
...
#        self.population = sorted(self.children, key=lambda x: x.value)[:self.size]
    ...
    def plot(self,generation=0):
        ...
        plt.xlim(-1,7)
        plt.ylim(-2,4)
        plt.savefig('64_B2_mutation_generation%03d.png'%(generation))
        plt.show()
        return
...
#            P.plot(generation=i)
            P.evolve()
        ''' compute the times that minimum found '''
